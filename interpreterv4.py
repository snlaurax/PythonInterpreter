...
class Interpreter(InterpreterBase):
    ...
    def run(self, program):
        ...
        main_func = self.__get_func_by_name("main", None, 0)
    ...
    def __get_func_by_name(self, name, obj_name, num_params):
        if name not in self.func_name_to_ast:
            closure_val_obj = self.env.get(name)
            if closure_val_obj is None:
                #Check if it is a method inside the object 
                for env in  self.env.environment:
                    for object_name, obj_value in env.items():
                        if object_name == obj_name and obj_value.type() == Type.OBJECT:
                            obj = obj_value.value()
                            # Check if the method exists in the object
                            result = obj.get(name)
                            if result:
                                closure = result
                                num_formal_params = len(closure.v.func_ast.get("args"))
                                ...
                                super().error(
                                    ...
                                    f"Method {name} not found in object {obj_name}",
                                )
                super().error(ErrorType.NAME_ERROR, f"Function {name} not found")
    ...
    def __call_func(self, call_ast):
        ...
        objref = call_ast.get("objref")
        new_env = {}

        if objref is not None:
            # Extract the object's name from the objref
            obj = self.env.get(objref)
            ...
            if obj is not None:
                ...
                super().error(
                    ErrorType.NAME_ERROR,
                    f"Object '{objref}' not found or method '{func_name}' doesn't exist.",
                )
            print (obj.type())
            if obj.type() != Type.OBJECT and func_name:
                super().error(
                    ErrorType.TYPE_ERROR,
                    f"'{objref}' is not an object.",
                )
        if func_name == "print":
        ...
        target_closure = self.__get_func_by_name(func_name, objref, len(actual_args))
    ...
    def __prepare_env_with_closed_variables(self, target_closure, temp_env):
        for captured_dict in target_closure.captured_env:
            for var_name, value in captured_dict.items():
    ...
    def __assign(self, assign_ast):
        ...
        src_value_obj_og = self.__eval_expr(assign_ast.get("expression"))
        src_value_obj = copy.copy(src_value_obj_og)

        # Check if var_name contains a "." indicating it's an object field
        if "." in var_name:
            obj_name, field_name = var_name.split(".")
            obj = self.env.get(obj_name)
            
            if obj is not None:
                if field_name == "proto" and (src_value_obj.type() == Type.OBJECT or src_value_obj.type() == Type.NIL):
                    obj.value().update("proto", src_value_obj_og)
                elif field_name == "proto" and src_value_obj.type() != Type.OBJECT:
                    super().error(
                        ErrorType.TYPE_ERROR,
                        f"'{src_value_obj} is not an object and therefore, cannot be the parent/child",
                    )
                else:
                    value = obj.v
                    value.update(field_name,src_value_obj)
            else:
                #if obj is not in the env, then type error 
                super().error(
                        ErrorType.TYPE_ERROR,
                        f"'{src_value_obj} is not an object and therefore, cannot be called",
                    )
                    
        else:
            # Regular assignment for non-object fields
    ...
    def __eval_name(self, name_ast):
        ...
        if "." in var_name:
            ...
            if obj is not None:
                value = obj.v
                result_get = value.get(field_name)
                if result_get == None: 
                        super().error(
                        ErrorType.NAME_ERROR,
                        f"Field '{field_name}' not found in object '{obj_name}' or object is not an instance of a class.",
                    )
                return result_get
            else:
                super().error(
                    ErrorType.NAME_ERROR,
                    f"Object '{obj_name}' not found or field '{field_name}' doesn't exist.",
                )
        else:
